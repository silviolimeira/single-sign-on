...
//		}
		...
		if (!Collections.list(request.getParameterNames()).contains(PARAMETER_LOGIN)
				|| !Collections.list(request.getParameterNames()).contains(PARAMETER_PASSWORD)) {
			System.out.println("No Parameters!");
			filterChain.doFilter(request, response);
			return;
		}

		//url for test: http://localhost:8080/user?login=Developer&password=password
		...
		UsernamePasswordAuthenticationToken authReq = new UsernamePasswordAuthenticationToken(login, null,
